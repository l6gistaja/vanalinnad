...
<MultiGeometry>
<xsl:for-each select="osm:FeatureCollection/gml:featureMember">
<xsl:if test="( osm:way/osm:geometryProperty/gml:LineString and not(osm:way/osm:highway) and not(osm:way/osm:waterway) and not(osm:way/osm:wood) and not(osm:way/osm:cables) and not(osm:way/osm:admin_level) and not(osm:way/osm:place) and not(osm:way/osm:wetland) and not(osm:way/osm:building) and not(osm:way/osm:power) and not(osm:way/osm:leisure) and not(osm:way/osm:barrier) and not(osm:way/osm:source_zoomlevel) and not(osm:way/osm:railway) and not(osm:way/osm:landuse) and not(osm:way/osm:natural and contains('wood,scrub,wetland', osm:way/osm:natural)) and not(osm:way/osm:source and contains(osm:way/osm:source,'Corine')) and not(contains('34600934,264633671,45487837,45531137,141101735,40169961,40181176,40181176,287935773,248270589,248270587,233276228', osm:way/@fid)) ) or contains('45478284,45478288,45478202,45478291,45478296', osm:way/@fid)">
<LineString><coordinates><xsl:value-of select="osm:way/osm:geometryProperty/gml:LineString/gml:coordinates"/></coordinates></LineString>
</xsl:if>
</xsl:for-each>
</MultiGeometry>
